...
<div class="modal-dialog" role="document">
    ...
    <form id="form">
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="penyakit" aria-describedby="emailHelp" placeholder="Enter here">
        </div>
        <div class="form-group">
            <label for="exampleInputEmail1">Keluhan</label>
            <input type="text" class="form-control" name="keluhan" aria-describedby="emailHelp" placeholder="Enter here">
        </div>
        <div class="form-group">
...
<script>
    ...
    function button(){
        $('.del').on('click',function(){
            ...
            $.ajax({
                type: "GET",
                url: "http://dihealth.dicampus.id/api.php?act=hapuspenyakit&&id=" + id,
    ...
    function loadData(){
        $.ajax({
            type: "GET",
            url: "http://dihealth.dicampus.id/api.php?act=datapenyakit",
            success: function (data) {
                ...
                data.forEach(e => {
                    template += `
                        <tr>
                        <th scope="row">${i}</th>
                        <td>${e.nama_penyakit}</td>
                        <td>${e.gejala}</td>
                        <td><a href='javascript:void(0)' data-id='${e.id_penyakit}' class='del'>Del</a></td>
                        </tr>
                    `;
